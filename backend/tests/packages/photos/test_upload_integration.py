...
async def test_confirming_a_batch_warms_the_rating_variant_in_the_real_edge_cache(
    ...
    try:
        # The real, direct-to-storage write a browser would perform against
        # the presigned grant -- never through the API. A PUT with the file
        # as the body, with the signed headers sent exactly as granted.
        ...
        # The router schedules this as a background task after responding;
        # awaited directly here since this test only cares about nginx's
        # cache afterward, not the deferral itself.
