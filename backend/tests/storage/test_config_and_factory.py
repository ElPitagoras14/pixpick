...
def test_naming_a_provider_explicitly_does_not_require_it_to_be_active(monkeypatch):
    """The migration check builds a port for a provider other than the
    active one, using that provider's own group of settings -- this only
    works because the two never share a field."""
    monkeypatch.setattr("src.storage.factory.storage_settings.storage_provider", "local")
