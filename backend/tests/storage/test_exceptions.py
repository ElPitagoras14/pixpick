"""Every storage failure reaches the rest of the app as one type, so a
consumer catches the base class and never has to know which ones exist."""
...
def test_a_missing_space_is_not_confused_with_an_unreachable_storage():
    """Two causes with two fixes: catching one never catches the other."""
