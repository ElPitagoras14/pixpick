...
answers with by itself, neither reachable by importing this project's code.
"""
...
async def test_the_grants_endpoint_has_its_own_stricter_ceiling(running_stack):
    """Task 4.2: granting exhausts its own, stricter zone well before the
    general one, and doing so never blocks a different operation from the
    same client -- the two zones count separately.

    Ordered before the general-ceiling test below on purpose: every test
    in this file shares one real client address (this machine's), so a
    burst that deliberately exhausts the *general* zone would leave it
    without the budget this test also needs, for as long as the zone
    takes to refill (request-throttling spec's own token-bucket
    behavior) -- exhausting only the *grants* zone here does not.
    """
...
async def test_a_burst_over_the_general_ceiling_is_rejected_with_the_common_body(running_stack):
    """Tasks 4.2, 4.3: a burst past the general zone's own budget gets
