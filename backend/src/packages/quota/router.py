...
# The instance's own consumption: a resource of its own and not a field on
# the account's, because it belongs to no one in particular -- any signed-in
# person reads the same answer.
instance_router = APIRouter(prefix="/instance")

# The account's own consumption, at a path that names no person: the answer
# is always about whoever is signed in, so there is no identifier here
# anyone could put someone else's in.
account_router = APIRouter(prefix="/account")

# The album level, under the album it is about -- the same shape the
# album's stats already use, the other owner-only resource of an album.
album_router = APIRouter(prefix="/albums/{album_id}")


@instance_router.get("/usage")
async def get_instance_usage(
    # Any signed-in person, and nothing more: no ownership to check, since
    # the instance's space belongs to no one account.
...
async def get_album_usage(
    ...
    # stranger gets `NotFoundError`. What an album occupies is its owner's
    # space and nobody else's -- the endpoint is what decides that, never a
    # condition inside the response.
