...
async def upsert_user(connection: AsyncConnection, identity: ExternalIdentity) -> UserRecord:
    ...
    data on every later one: identity is established by the (provider,
    provider_user_id) pair, never by email, which is why that's the conflict
    target and not a column of its own.
    """
    row = await fetch_one(
        connection,
        """
...
async def user_exists(connection: AsyncConnection, *, user_id: UUID) -> bool:
    """Without a lock: granting no longer serializes on this row -- the
    instance's own advisory lock covers that now -- so this only has to
    answer whether a session's account is still there, the same question
    `lock_user_id` used to answer while it locked the row for the rest of
    the transaction.
    """
...
async def delete_expired_sessions_for_user(connection: AsyncConnection, user_id: UUID) -> None:
    """Housekeeping, not correctness: an expired row never authenticates
    regardless of whether it was ever deleted."""
    await write(
        connection,
...
async def get_user_by_session_token_hash(
    ...
    """A session that doesn't exist and one that expired come back the same
    way: nothing -- the caller treats both identically."""
