...
class StorageNotReadyError(StorageError):
    """Raised when the storage answers but the space this project keeps
    its objects in isn't there.

    Its own type, not `StorageUnavailableError` (D3): the two are fixed
    differently -- this one by creating the bucket at the provider, that
    one by looking at the network or the service -- so sharing a type
    would send the reader of a startup error to the wrong place.
    """
