...
# What it answers when something created the bucket between the check and
# the create call.
...
def _client(endpoint: str):
    return boto3.client(
        ...
        # Explicit, not left to boto3's auto-detection: the storage hostname
        # is a subdomain of the application's, which looks enough like a
        # virtual-hosted bucket to be guessed wrong. Path style is what
        # nginx's storage block forwards untouched.
        config=Config(signature_version=_SIGNATURE_VERSION, s3={"addressing_style": "path"}),
    )


class MinioStorageAdapter:
    """Two clients, not one: grants are signed against the address the
    browser reaches, queries and deletes go through the server's."""
    ...
    async def ensure_ready(self) -> None:
        """Creates the bucket when missing: this is the provider the project
        runs itself, the same guarantee `migrate` gives the schema."""
        if await self._bucket_exists():
            return
        try:
            await asyncio.to_thread(self._server_client.create_bucket, Bucket=self._bucket)
        except ClientError as exc:
            # Created between the check and this call: ready either way.
    ...
    def grant_upload(
        ...
        *,
        ...
        try:
            ...
            # POST with a policy, because no presigned URL can bound a size
            # range -- this signs the object identity and content type only.
