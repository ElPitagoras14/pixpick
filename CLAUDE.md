...
- Si después de devolver el control el usuario pide correcciones o ajustes sobre ese mismo change y todavía no se commiteó nada, ese trabajo también se cronometra: correr `date` apenas arranca la corrección (no asumir que alcanza con estimarlo después) y agregarlo como una subsección dentro de la sección de ese change en `local/tiempos.md` (no una entrada nueva), con sus propios cortes y las mismas pausas restadas.
  - Esto ya pasó por alto dos veces (`add-media-ports-and-local-adapters` y `add-albums-and-upload`, documentado en `local/tiempos.md`): correr `date` es el primer paso, antes de leer o tocar cualquier archivo para atender el pedido — no un paso que se hace "cuando se acuerde" a mitad de la corrección.
