...
La cantidad de fotos que un álbum admite SHALL tener un máximo declarado en la configuración del entorno. El máximo SHALL evaluarse al conceder: si el lote pedido haría que el álbum superara el máximo, SHALL rechazarse el lote completo y la respuesta SHALL indicar cuánto espacio queda, para que quien pide pueda ajustar la selección.
...
#### Scenario: Un lote que excede el máximo se rechaza completo

- **WHEN** se pide un lote que haría que el álbum superara el máximo
- **THEN** no se concede permiso para ninguna foto del lote
- **AND** la respuesta indica cuánto espacio queda disponible
...
- **THEN** el segundo pedido se rechaza
